...
% phi equation
Jy(1:n) = (M - lamb*F)*y(1:n) + diag(phi)*y(4*n+2:5*n+1) + ...
         (-lamb*diag(phi))*y(5*n+2:6*n+1)                + ...
          MD*y(6*n+2:7*n+1) + (-F*phi)*y(8*n+2);
...
FunX = zeros(n,1);
for i = 1:n
    FunXepsY(i) = (rho(i)+epsilon*y(3*n+1+i)) - XSteam('rho_pT',P,T(i)+epsilon*y(2*n+1+i));
    FunX(i) = rho(i) - XSteam('rho_pT',P,T(i));
end
Jy(3*n+2:4*n+1) = (FunXepsY - FunX)/eps;
